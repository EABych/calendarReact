...
class Home extends React.Component {
    ...
    UNSAFE_componentWillMount(){
        fetch('http://localhost:3002/allEvents')
            ...
            .then(result => {
                const newAllEvents = renameKeys(result, false);
...
c
    ...
    addNewEvent() {
        ...
        if (Object.keys(newEvent).length) {
            ...
            const newEventRename = renameKeys(newEvent, true);
            allEvent.push(newEvent);
            return fetch('http://localhost:3002/postNewEvent', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify(newEventRename),
            })
                .then((res) => {
                    this.setState({
                        allEvent: allEvent,
    ...
    editEvent(e) {
        const newAllEvent = findActiveEvent(this.state.allEvent, e.target.id, true)[0];
    ...
    saveEditEvent() {
        ...
        if (Object.keys(newEvent).length) {
            ...
            const body = {
                newEvent: newEvent,
                id : this.state.activeEvent._id
            };

            console.log('@@@@@ newEvent, newAllEvent', newEvent, newAllEvent, this.state.activeEvent._id);


            return fetch('http://localhost:3002/modificationEvent', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    deleteEvent(e) {
       ...
       const newAllEvent = findActiveEvent(this.state.allEvent, e.target.id)
        console.log('@@@@@newAllEvent ', newAllEvent);

        return fetch('http://localhost:3002/deleteEvent', {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
